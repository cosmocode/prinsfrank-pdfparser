...
/**
 * Orientation-agnostic, two-pass line grouping.
 *
 ...
 * {@see ANGLE_TOLERANCE_DEGREES}) and their glyph extents overlap by at least {@see OVERLAP_PERCENTAGE} measured
 * along the baseline normal. For horizontal text the baseline normal is the page Y axis, so this reduces to a
 * plain Y-overlap test, including keeping an enclosed subscript (the "2" in "CO2") on its line. Within a line the
 * text elements are ordered along the baseline's own direction vector, so reading order follows the text's advance
 * direction at any rotation (90 and 270 degree text each read the right way round).
 *
 ...
 *
 * For horizontal text the baseline normal is the page Y axis and every line falls into a single orientation
 * bucket, so an upright page groups exactly as a fixed top-to-bottom, left-to-right pass would; the baseline
 * geometry only changes the outcome where text is rotated.
 */
class BaselineClusterStrategy {
    /** @var int<0, 100> minimum percentage two glyph extents must overlap, measured across the baseline, to share a line */
    private const OVERLAP_PERCENTAGE = 90;

    /** @var float maximum angle between two baselines for them to count as the same direction, and so be eligible to share a line or block */
    private const ANGLE_TOLERANCE_DEGREES = 3.0;

    /** @var float how many line heights two stacked lines may be apart, measured along the baseline normal, before they are treated as separate blocks */
    private const BLOCK_GAP_LINE_HEIGHTS = 2.5;
    ...
    public static function group(array $positionedTextElements): iterable {
        ...
        $cosTolerance = cos(deg2rad(self::ANGLE_TOLERANCE_DEGREES));

        $lines = self::clusterIntoLines($positionedTextElements, $cosTolerance);
        foreach (self::orderBlocks(self::segmentIntoBlocks($lines, $cosTolerance)) as $block) {
            foreach (self::orderLinesWithinBlock($block) as $line) {
                yield $line->elements;
            }
        }
    }
     ...
     * points nearly the same way (within {@see ANGLE_TOLERANCE_DEGREES}) and its glyph extent overlaps the seed's
     * by at least {@see OVERLAP_PERCENTAGE} measured along the baseline normal, or it is an enclosed subscript of
     * the line. Within a line the runs are ordered along the baseline (reading direction); each Line carries the
     * seed's baseline direction and origin, its height, its highest point on the page, and its document position.
     *
     * @param list<PositionedTextElement> $positionedTextElements
     * @param float                       $cosTolerance cosine of {@see ANGLE_TOLERANCE_DEGREES}; see {@see group()}
     * @return list<Line>
     */
    private static function clusterIntoLines(array $positionedTextElements, float $cosTolerance): array {
        $documentOrder = [];
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            for ($j = $i + 1; $j < $count; $j++) {
                ...
                $belongsOnLine = $smallestHeight !== 0.0 && $overlap / $smallestHeight * 100 >= self::OVERLAP_PERCENTAGE;
     ...
     * split along its baseline normal wherever the gap between consecutive lines exceeds
     * {@see BLOCK_GAP_LINE_HEIGHTS} line heights -- a paragraph break, or a region stacked above or below, rather
     * than the next line of a paragraph. The lines are ordered into reading order later, per block, by
     * {@see orderLinesWithinBlock()}.
     *
     ...
     * {@see clusterIntoLines()} and read across the columns row by row. The only side-by-side separation this
     * makes is by orientation, in the bucketing above. Multi-column layout is a known limitation.
     *
     * @param list<Line> $lines
     * @return list<Block>
     */
    private static function segmentIntoBlocks(array $lines, float $cosTolerance): array {
        ...
        foreach ($buckets as $bucket) {
            ...
            foreach ($bucket as $line) {
                ...
                if ($previousPosition !== null
                    && $previousPosition - $position > max($previousHeight, $line->height) * self::BLOCK_GAP_LINE_HEIGHTS
                ) {
     ...
     * @return list<Line>
     */
    private static function orderLinesWithinBlock(Block $block): array {
        $lines = $block->lines;
        if (count($lines) <= 1) {
            return $lines;
        }
     ...
     * @return list<Block>
     */
    private static function orderBlocks(array $blocks): array {
        usort(
            $blocks,
     ...
     * element's baseline. The gap is taken along the baseline direction, so it stays positive in reading order and
     * the test holds at any rotation; for horizontal text it reduces to comparing page-X offsets.
     *
     * @param Font $font the font the advance of $previous is measured with
     * @throws PdfParserException
     */
    public static function requiresSpaceBetween(PositionedTextElement $previous, PositionedTextElement $current, Font $font): bool {
        ...
        // of the em separates word breaks from kerning.
        $advanceWidth = $previous->getAdvanceWidth($font, $unit);
