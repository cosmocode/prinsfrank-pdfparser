...
class TextOverlapStrategyTest extends TestCase {
    ...
    public function testInsertsSpaceForAGapWiderThanTheReconstructedAdvance(): void {
        ...
        $previous = new PositionedTextElement('(A)', new TransformationMatrix(1, 0, 0, 1, 300, 700), new TextState(DictionaryKey::FONT, 10, resourceChain: new ResourceDictionaryChain([$resourceDictionary])));
        $farRight = new PositionedTextElement('(B)', new TransformationMatrix(1, 0, 0, 1, 400, 700), new TextState(DictionaryKey::FONT, 10));
        $justRight = new PositionedTextElement('(B)', new TransformationMatrix(1, 0, 0, 1, 305, 700), new TextState(DictionaryKey::FONT, 10));

        static::assertTrue($strategy->requiresSpaceBetween($previous, $farRight, $document));
        static::assertFalse($strategy->requiresSpaceBetween($previous, $justRight, $document));
    }
}
